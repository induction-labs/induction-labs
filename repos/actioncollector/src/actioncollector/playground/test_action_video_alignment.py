...
def show_frame(ts_ms):
    ...
    if idx >= len(actions): idx = len(actions) - 1
    action = actions[idx]

    cv2.circle(img, (int(action['x']), int(action['y'])), 10, (0, 0, 255), 2)
    cv2.putText(img, f"Frame: {frame_time:.1f} ms", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    cv2.putText(img, f"Action: {action['timestamp']:.1f} ms", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
...
cv2.createTrackbar('Time (ms)', 'Video with Cursor Overlay', 0, int(range_ms), lambda v: set_position(v))
