...
class Qwen25OActionLITTesting(
    BaseLITModule[MODEL_TYPE, ActionDataSample, "Qwen25oActionTestingConfig"]
):
    """
    ...
    def compute_loss(self, outputs_actions: torch.Tensor, inputs: ActionDataSample):
        """
    ...
    def run_validation_step(self, inputs: ActionDataSample, global_step: int):
        # Forward pass through the model
        action_outputs = self.model_test(self.hidden_states)
        (
            loss,
            predicted_xs,
            predicted_ys,
            ...
            output_actions,
            cursor_path,
        ) = self.compute_loss(action_outputs, inputs)
        ...
        action_outputs = action_outputs[inputs.action_tokens]
...
class Qwen25oActionTestingConfig(BaseModuleConfig):
    """
    ...
    config_path: str = (
        "modeling.modules.action_instruct.action_head_only.Qwen25oActionTestingConfig"
    )
    ...
    ) -> Qwen25OActionLITTesting:
        return Qwen25OActionLITTesting(
            self, run_config, runtime_config, instance_config
        )
